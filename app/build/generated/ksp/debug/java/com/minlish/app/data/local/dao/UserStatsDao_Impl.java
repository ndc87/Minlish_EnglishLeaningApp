...
public final class UserStatsDao_Impl implements UserStatsDao {
  ...
  @Override
  public Object getUserStatsOnce(final String userId,
      final Continuation<? super UserStatsEntity> $completion) {
    final String _sql = "SELECT * FROM user_stats WHERE userId = ?";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 1);
    int _argIndex = 1;
    _statement.bindString(_argIndex, userId);
    final CancellationSignal _cancellationSignal = DBUtil.createCancellationSignal();
    return CoroutinesRoom.execute(__db, false, _cancellationSignal, new Callable<UserStatsEntity>() {
      @Override
      @Nullable
      public UserStatsEntity call() throws Exception {
        final Cursor _cursor = DBUtil.query(__db, _statement, false, null);
